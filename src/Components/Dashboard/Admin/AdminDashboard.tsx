...
const AdminDashboard = ({ user }: { user: User }) => {
  ...
  const dailyUsers = getDataForDates(
    dailyStats?.data.dailyNewUsers,
    allDates,
    'newUsers'
  );
  const dailyPosts = getDataForDates(
    dailyStats?.data.dailyNewPosts,
    allDates,
    'newPosts'
  );
  const dailyComments = getDataForDates(
    dailyStats?.data.dailyNewComments,
    allDates,
    'newComments'
  );
  const dailyPremiumContent = getDataForDates(
    dailyStats?.data.dailyNewPremiumContent,
    allDates,
    'newPremiumContent'
  );
  ...
  return (
    <div className="py-10 bg-background-dark">
      <CustomContainer>
        <div className="relative min-h-[50vh] z-10 flex flex-col justify-center items-center rounded-xl overflow-hidden">
          <Image
            alt="cover image"
            src={adminDashboard}
            layout="fill"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 md:gap-4 my-10">
